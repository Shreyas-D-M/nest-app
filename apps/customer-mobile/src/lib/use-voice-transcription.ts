...
export function useVoiceTranscription({ onTranscript, onError }: Options) {
  ...
  const stopListening = useCallback(async () => {
    ...
    reset('idle', 'Voice recording stopped');
  }, [reset]);
  ...
  const startRecording = useCallback(async () => {
    ...
    try {
      ...
      if (permission.status !== 'granted') {
        ...
        onError('Microphone permission required', 'Allow microphone access, then try voice input again.');
        return;
      }
  ...
  const finishAndTranscribe = useCallback(async () => {
    ...
    try {
      ...
      if (session !== sessionRef.current || (error instanceof Error && error.name === 'AbortError')) return;
  ...
  return { voiceUiState, voiceStatus, isRecording: voiceUiState === 'recording', toggleVoice, stopListening };
}
